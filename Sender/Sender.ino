...
void loop() {
  ...
  String sensors_reading[] = {reverse(s[0]), normal(s[1]), normal(s[2]), normal(s[3]), normal(s[4])}; // Creating a dictionary where we have stored each value, 
                                                                                        // with reversed values included
  //Data Transmission to the python code via serial port
  char Read[100];
  Serial.readBytesUntil('A', Read, 100);
  String Info = "  " + String(sensors_reading[0]) + "  " + String(sensors_reading[1]) + "  " +  String(sensors_reading[2]) + "  " + String(sensors_reading[3]) + "  " + String(sensors_reading[4]) + "  ";
